...
public class UsingOccupationsImpl  implements UsingOccupations{
    ...
    @Override
    @Transactional
    public IdNameListResponse findUsingOccupationsIdForCode(IdNameListResponse response, String nameCode) {
        ...
        for (IdNameResponse item : idNameResponses) {
            ...
            criteriaRtCode.add(Restrictions.eq("rtCode." + nameCode, item.getId()));
    ...
    @Override
    public IdNameListResponse findUsingOccupationsIdForDcDutiesMustKnow(IdNameListResponse response) {

        Criteria criteriaRtCode = hibernateTemplate.getSessionFactory().getCurrentSession().createCriteria(RtDutiesMustKnowEntity.class, "rtDutiesMustKnow");

        List<IdNameResponse> idNameResponses = response.getIdNameResponses();

        for (IdNameResponse item : idNameResponses) {

            criteriaRtCode.add(Restrictions.eq("dcDutiesMustKnowId", item.getId()));
